...
Type
  ...
  private
    ...
    Function ReadObjectLiteral(const NameCase: TCaseType): String;
    Procedure ReadObjectSeparator;
  public
...
Procedure TJsonObjectArrayParser.SkipWhitespace;
begin
  while (not Reader.EndOfStream) and (Char(Reader.Peek) in [#10,#13,#32]) do Reader.Read;
end;

Function TJsonObjectArrayParser.ReadObjectLiteral(const NameCase: TCaseType): String;
// Reads the characters of the object literal, applying the name case to the
// names at the object's top level
begin
  Result := '';
  var Name := true;
  var Escape := false;
...
Procedure TJsonObjectArrayParser.ReadObjectSeparator;
begin
  SkipWhitespace;
  if not Reader.EndOfStream then
  begin
...
begin
  ...
  begin
    ...
    if (not Reader.EndOfStream) and (Char(Reader.Peek) = '{') then
      Result := ReadObjectLiteral(NameCase)
    else
      raise Exception.Create('Invalid Json-array');
    ReadObjectSeparator;
  end else
